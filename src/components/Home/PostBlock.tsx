          ...
          <div className={styles.onePostBlock}>
            <div className={styles.personalBlock}>
              <div className={styles.firstBlock}>
                <img
                  src={!item.user.image ? noimage : `http://localhost:4000/${item.user.image}`}
                  alt=""
                />
                <div>
            ...
            <div className={styles.onePost}>
              ...
              {item.image.map((item) => {
                return (
                  <img className={styles.image} src={`http://localhost:4000/${item.path}`} alt="" />
                );
              })}
              <LikesAndComments userId={item.user._id} postId={item._id} like={item.likes}/>
            </div>
          </div>
        );
      })}
